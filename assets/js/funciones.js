...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Inicializa el plugin DataTable en el elemento con ID "tbl".
    $('#tbl').DataTable();
    
    // Agrega un controlador de eventos para formularios con la clase "confirmar" cuando se envían.
    $(".confirmar").submit(function (e) {
        // Previene el comportamiento predeterminado del formulario, que es enviar la solicitud al servidor.
        e.preventDefault();
        
        // Muestra un cuadro de diálogo de confirmación utilizando la biblioteca Swal (SweetAlert).
        Swal.fire({
            ...
            if (result.isConfirmed) {
                // Si se confirma la eliminación, se envía el formulario actual.
                this.submit();
            }
        })
    })

    // Inicializa el componente de autocompletado en un campo de entrada con el ID "nom_cliente".
$("#nom_cliente").autocomplete({
    // Establece la longitud mínima de caracteres antes de activar el autocompletado.
    minLength: 3,

    // Configura la fuente de datos para el autocompletado mediante una función.
    source: function (request, response) {
        $.ajax({
            // URL del script PHP o servicio que proporciona los datos para el autocompletado.
            url: "ajax.php",

            // Especifica el tipo de datos esperados en la respuesta (JSON en este caso).
            dataType: "json",

            // Datos que se enviarán al servidor para realizar la búsqueda.
            data: {
                q: request.term // "q" es el término de búsqueda ingresado por el usuario.
            },

            // Función que se ejecuta cuando la solicitud AJAX es exitosa.
            success: function (data) {
                response(data); // La función "response" se llama con los resultados para mostrar.
            }
        });
    },

    // Función que se ejecuta cuando se selecciona un elemento del autocompletado.
    select: function (event, ui) {
        // Actualiza los campos con los datos seleccionados del elemento autocompletado.
        $("#idcliente").val(ui.item.id); // Establece el valor del campo con ID "idcliente".
        $("#nom_cliente").val(ui.item.label); // Establece el valor del campo con ID "nom_cliente".
        $("#tel_cliente").val(ui.item.telefono); // Establece el valor del campo con ID "tel_cliente".
        $("#dir_cliente").val(ui.item.direccion); // Establece el valor del campo con ID "dir_cliente".
    }
});
