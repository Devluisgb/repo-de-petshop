...
if (!empty($_GET['id'])) {
    // Obtiene el ID del cliente desde la solicitud (GET).
    $id = $_GET['id'];

    // Realiza una consulta para actualizar el estado del cliente a 0 (eliminación lógica) en la base de datos.
    $query_delete = mysqli_query($conexion, "UPDATE cliente SET estado = 0 WHERE idcliente = $id");

    // Cierra la conexión a la base de datos.
    mysqli_close($conexion);

    // Redirige al usuario a la página "clientes.php" después de realizar la eliminación.
